...
O大 是 AI Scrum Team 的**第 9 位成員**，不是通知機器人，是有個性的 AI 隊友。
...
| 週三 10:00 | 🌡️ 週中健康檢查 | 進度達成率 + Yellow Alert 工項 |
| 週五 17:00 | 🏁 週收尾報告 | 完成率 + 燃盡圖快照 + 下週預告 |
| 週五 17:15 | 📝 週報草稿 | 自動產出 → PMO 確認 → 發出（存 Notion）|
| **週五 00:00 起** | 📊 Claude 使用量統計開始 | 統計區間啟動：上週五 00:00 ～ 本週四 23:59 |
| **週四 23:00** | 📊 Claude 使用量彙整推播 | 統計截止，彙整全員使用量報告推播至 LINE |

### Claude 使用量週報（每週四 23:00 推播）
...
```
方式 A｜主動填報（Phase 1）
  O大 每週四 20:00 在 LINE 發問：
  「本週 Claude 使用情況，跟我說一下：
    1. 用了幾天？
    2. 用了哪些工具？
    3. 主要拿來做什麼？」
  → 成員回覆 → O大 彙整 → 23:00 推播報告

方式 B｜自動讀取（Phase 2，需 API 權限）
  串接 Anthropic Usage API（如有開放）
...
Notion / AI 週報資料庫
  ...
  │    ├─ OpenAI Responses API 日落確認
  │    └─ 金管會 AI 指引草案
  ├─ 2026-W18（4/27 ～ 5/3）
  │    └─ ...
  └─ 標籤索引（#Claude / #OpenAI / #法規 / #RAG）
```
...
每日自動
  ...
  🦀 17:20  IDA Pulse 確認（@未填成員）
  📊 17:45  每日工作總結（GitHub Board）
  🔴 18:00  逾期 Alert + IDA Pulse 最終通知
